			...
			if context["error"] == nil {
				...
				if err != nil {
					...
					if len(errors) == 0 {
						...
						cuser.Save(users)
						return c.Status(302).JSON(fiber.Map{"redirectURL": "/"})
					}
				}
			}
		}
...
func APINewPassword(db_ *db.DB, urls ...interface{}) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if len(password1) < 8 {
			return c.SendString(`{"status": "500", "errors": {"password1": "Слишком короткий пароль"}}`)
		}
		if len(password2) < 8 {
			return c.SendString(`{"status": "500", "errors": {"password2": "Слишком короткий пароль"}}`)
		}
		if password1 != password2 {
			return c.SendString(`{"status": "500", "errors": {"password1": "Пароли не совпадают"}}`)
		}
